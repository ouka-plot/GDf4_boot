...
int mqtt_publish_json(const char *topic, const char *json_data, uint8_t qos, uint8_t retain)
{
    ...
    // 使用 AT+MQTTPUBRAW 发送原始JSON数据（不需要转义）
    // 格式：AT+MQTTPUBRAW=<LinkID>,<topic>,<length>,<qos>,<retain>
    snprintf(cmd, sizeof(cmd), "AT+MQTTPUBRAW=0,\"%s\",%u,%u,%u\r\n", 
             topic, json_len, qos, retain);
    
    // 发送命令，等待 > 提示符
    if (!ESP8266_SendCmd(cmd, ">", 5000)) {
        u0_printf("[MQTT] PUBRAW prompt failed\r\n");
        return -1;
    }
    
    // 发送原始JSON数据
    u1_send_string((char *)json_data);
    u0_printf("[DATA] %s\r\n", json_data);
    
    // 等待发送完成确认
    if (!ESP8266_WaitResp("OK", 10000)) {
        u0_printf("[MQTT] Publish JSON to '%s' failed\r\n", topic);
        return -1;
    }
    
    u0_printf("[MQTT] Publish JSON OK (topic=%s)\r\n", topic);
    return 0;
}
...
void mqtt_poll(void)
{
    while (u1_ucb.out != u1_ucb.in) {
        ...
        /* 检测MQTT断线和重连事件 */
        if (strstr(tmp, "+MQTTDISCONNECTED:") != NULL) {
            u0_printf("[MQTT] Broker disconnected, auto-reconnecting...\r\n");
            continue;
        }
        if (strstr(tmp, "+MQTTCONNECTED:") != NULL) {
            u0_printf("[MQTT] Broker reconnected!\r\n");
            continue;
        }
